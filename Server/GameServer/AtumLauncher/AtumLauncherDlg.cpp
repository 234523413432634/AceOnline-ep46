...
#include <urlmon.h>					// 2014-02-19 by jhseol, 베트남 X-Trap 키 구분 못하는 현상 수정
#pragma comment(lib, "urlmon")		// 2014-02-19 by jhseol, 베트남 X-Trap 키 구분 못하는 현상 수정
#pragma comment(lib, "wininet")		// 2014-02-19 by jhseol, 베트남 X-Trap 키 구분 못하는 현상 수정
...
#define EXE2_BG_RESOLUTION_COMBOBOX_WIDTH		200		// 해상도 선택 Combo Box Width
	...
	#define EXE2_BG_WINDOWSMODE_CHECKBOX_POS_X		700		// 2007-12-27 by cmkwon, 윈도우즈 모드 기능 추가 - Windows Mode Check Box 위치 X
	...
	#define EXE2_BG_REMEMBERID_CHECKBOX_POS_X		669		// 2013-03-25 by jhseol, 게임포지 영국 ID 체크버튼 위치 수정 기존 위치(689)에서 -20 만큼
#else
	#define EXE2_BG_REMEMBERID_CHECKBOX_POS_X		689		// Remember ID CheckBox 위치 X
#endif
...
#else
	#define EXE2_BG_WINDOWSMODE_CHECKBOX_POS_X		715		// 2007-12-27 by cmkwon, 윈도우즈 모드 기능 추가 - Windows Mode Check Box 위치 X
...
//#if defined(SERVICE_TYPE_KOREAN_SERVER_1) || defined(SERVICE_TYPE_THAI_SERVER_1) || defined(SERVICE_TYPE_ENGLISH_SERVER_2) || defined(SERVICE_TYPE_GERMAN_SERVER_1) || defined(SERVICE_TYPE_TURKISH_SERVER_1) || defined(SERVICE_TYPE_FRENCH_SERVER_1) || defined(SERVICE_TYPE_ITALIAN_SERVER_1) || defined(SERVICE_TYPE_POLISH_SERVER_1) || defined(SERVICE_TYPE_SPANISH_SERVER_1)
	...
	// 2013-09-26 by bckim, 캐나다 계정기억하기
#if defined(SERVICE_TYPE_ENGLISH_SERVER_1)  // 캐나다 체크박스 위치 수정 
...
#endif	
	// End. 2013-09-26 by bckim, 캐나다 계정기억하기
...
// 2009-01-30 by cmkwon, 러시아 Innova 런처 시스템(프로스트) 수정 - 아래와 같이 수정함.
						///////////////////////////////////////////////////////////////////////////////
						// 2009-01-30 by cmkwon, 러시아 Innova 런처 시스템(프로스트) 수정 - 아래와 같이 수정함.
						sprintf(szCmdParam, "%s %d %s %d %s %s %d %s INET %s %s %s %s %d %d"	// 2014-01-06 by jhseol, 테섭 접속 여부(TRUE=테섭, FALSE=본섭)	// 2011-12-21 by hskim, EP4 [동영상 1회 재생]
							...
							, pConnOK->ConnectToTestServer);		// 2014-01-06 by jhseol, 테섭 접속 여부(TRUE=테섭, FALSE=본섭)
...
// 2009-01-30 by cmkwon, 러시아 Innova 런처 시스템(프로스트) 수정 - 아래와 같이 수정함.
						...
						sprintf(&(szCmdParam[strlen(szCmdParam)]), " %s %s %s %s %s %s %d %d"	// 2014-01-06 by jhseol, 테섭 접속 여부(TRUE=테섭, FALSE=본섭)	// 2011-12-21 by hskim, EP4 [동영상 1회 재생]
							, g_szArrargv[1], g_szArrargv[2], g_szArrargv[3]
							, g_szArrargv[4], g_szArrargv[5], g_szArrargv[6], pConnOK->OpeningMoviePlay			// 2011-12-21 by hskim, EP4 [동영상 1회 재생]
							, pConnOK->ConnectToTestServer);		// 2014-01-06 by jhseol, 테섭 접속 여부(TRUE=테섭, FALSE=본섭)
						sprintf(cmdLine, "%s %s", szAppPath, szCmdParam);
...
// 2009-01-30 by cmkwon, 러시아 Innova 런처 시스템(프로스트) 수정 - 아래와 같이 수정함.
						///////////////////////////////////////////////////////////////////////////////
						// 2009-01-30 by cmkwon, 러시아 Innova 런처 시스템(프로스트) 수정 - 아래와 같이 수정함.
 						sprintf(szCmdParam, "%s %d %s %d %s %s %d %s %d %d"		// 2014-01-06 by jhseol, 테섭 접속 여부(TRUE=테섭, FALSE=본섭)
							, pConnOK->FieldServerIP, pConnOK->FieldServerPort, pConnOK->IMServerIP, pConnOK->IMServerPort, m_szAccountName, szEncodedString, m_nWindowModeReg, szTmpWindowDegree, pConnOK->OpeningMoviePlay	// 2011-12-21 by hskim, EP4 [동영상 1회 재생]
							, pConnOK->ConnectToTestServer);		// 2014-01-06 by jhseol, 테섭 접속 여부(TRUE=테섭, FALSE=본섭)
						sprintf(cmdLine, "%s %s", szAppPath, szCmdParam);
...
// 2009-01-30 by cmkwon, 러시아 Innova 런처 시스템(프로스트) 수정 - 아래와 같이 수정함.
						///////////////////////////////////////////////////////////////////////////////
						// 2009-01-30 by cmkwon, 러시아 Innova 런처 시스템(프로스트) 수정 - 아래와 같이 수정함.
						sprintf(szCmdParam, "%s %d %s %d %s %s %d %s %d %d DEVELOP"		// 2014-01-06 by jhseol, 테섭 접속 여부(TRUE=테섭, FALSE=본섭)
							, pConnOK->FieldServerIP, pConnOK->FieldServerPort, pConnOK->IMServerIP, pConnOK->IMServerPort, m_szAccountName + m_szCrocessSuffix, szEncodedString, m_nWindowModeReg, szTmpWindowDegree, pConnOK->OpeningMoviePlay	// 2011-12-21 by hskim, EP4 [동영상 1회 재생]
							, pConnOK->ConnectToTestServer);		// 2014-01-06 by jhseol, 테섭 접속 여부(TRUE=테섭, FALSE=본섭)
						sprintf(cmdLine, "%s %s", szAppPath, szCmdParam);
						...
						{
							char buf[128];
							sprintf(buf, "%s %d %s %d %s %s %d %s %d %d DEVELOP",	// 2014-01-06 by jhseol, 테섭 접속 여부(TRUE=테섭, FALSE=본섭)	// 2011-12-21 by hskim, EP4 [동영상 1회 재생]
												pConnOK->FieldServerIP, pConnOK->FieldServerPort,
												pConnOK->IMServerIP, pConnOK->IMServerPort,
						...
						if (m_szCrocessSuffix == "")
						{
							// 2009-01-30 by cmkwon, 러시아 Innova 런처 시스템(프로스트) 수정 - 아래와 같이 수정함.
							//ExecGame(cmdLine);
							ExecGame(cmdLine, szAppPath, szCmdParam, pConnOK->ConnectToTestServer);		// 2014-01-06 by jhseol, 테섭 접속 여부(TRUE=테섭, FALSE=본섭)
						}
						else
						{
							// 크로세스를 통해 실행
...
//						}
						...
						for (int yi = 0; yi < pServerListOK->NumOfServerGroup; yi++)
						{
							...
							if ( 0 == nChannelingNum )												// 문피아 체널링이 아닐경우
							{
								if ( 0 == strncmp("03", pMexServerGroup->ServerGroupName, 2) )		// 03번 문피아 서버면 skip
								{
									continue;
								}
							}
							...
							{
								if ( 0 == strncmp("01", pMexServerGroup->ServerGroupName, 2)		// 01번 데카 서버면 skip
									|| 0 == strncmp("02", pMexServerGroup->ServerGroupName, 2) )	// 02번 비숍 서버면 skip
								{
									continue;
								}
							}
...
// 2012-07-11 by hskim, 선택적 셧다운
	...
	{
		AtumMessageBox(STRMSG_S_ATUMLAUNCHER_0038);		// 2012-07-11 by hskim, 선택적 셧다운
...
void CAtumLauncherDlg::ExecGame(char *cmdLine, char *i_szAppPath/*=NULL*/, char *i_szCmdParam/*=NULL*/, BOOL i_bConnectToTestServer/*=FALSE*/)	// 2014-01-06 by jhseol, 테섭 접속 여부(TRUE=테섭, FALSE=본섭))
{
#ifdef _ATUM_DEVELOP
...
// 2012-02-09 by hskim, 베트남 X-Trap -> VTC 가드로 변경
	...
	if ( TRUE == i_bConnectToTestServer )		// 2014-01-06 by jhseol, 테섭 접속 여부(TRUE=테섭, FALSE=본섭)
	{
		XTrap_L_Patch(XTRAP_KEY_STRING_TEST, NULL, 60);
		DbgOut("Launcher Connect TestServer\r\n");		// 2014-02-19 by jhseol, 베트남 X-Trap 키 구분 못하는 현상 수정
	}
	else
	{
...
#if defined(SERVICE_TYPE_ARGENTINA_SERVER_1)		// 2012-02-09 by hskim, 베트남 X-Trap -> VTC 가드로 변경
	///////////////////////////////////////////////////////////////////////////////
	// 2009-10-29 by cmkwon, 베트남 X-TRAP 자동업데이트 기능 적용 - CAtumLauncherDlg::ExecGame#,
